...
class IDetect(nn.Module):
    ...
    def forward(self, x):
        '''
        前向传播：
        return: 是否在训练,否则带上框的位置和分类信息
        '''
        # x = x.copy()  # for profiling
        z = []  # 推理输出
        self.training |= self.export    # 查看是否在训练状态
        for i in range(self.nl):
...
class Model(nn.Module):
    ...
    def forward_once(self, x, profile=False, extra_features: list = []):
        ...
        if len(extra_features) != 0 and len(extra_features) == len(features):
            return x, features
...
def parse_model(d, ch):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        if m in [nn.Conv2d, Conv, RobustConv, RobustConv2, DWConv, GhostConv, RepConv, RepConv_OREPA, DownC, 
                 ...
                 Yolov7_Tiny_E_ELAN, Yolov7_Tiny_SPP, Yolov7_E_ELAN, Yolov7_E_ELAN_NECK,Yolov7_E_ELAN_NECK_ACMIX,
                 Yolov7x_E_ELAN, Yolov7_Tiny_E_ELAN_Attention, Yolov7_Tiny_E_ELAN_DCN, Yolov7_Tiny_E_ELAN_ACMIX,
